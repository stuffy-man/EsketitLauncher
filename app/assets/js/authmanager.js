...
// ─── Офлайн-аккаунты (пиратка) ──────────────────────────────────────────────

const VALID_USERNAME = /^[a-zA-Z0-9_]{1,16}$/

/**
 * Рассчитать офлайн-UUID по нику — тем же алгоритмом, что и ванильный сервер
 * в offline-режиме: UUID v3 из MD5("OfflinePlayer:" + ник).
 *
 * @param {string} username Ник игрока.
 * @returns {string} 32-символьный UUID без дефисов.
 */
...
/**
 * Добавить офлайн («пиратский») аккаунт по одному нику, без пароля.
 *
 * @param {string} username Ник игрока (1–16 символов: буквы, цифры, «_»).
...
exports.validateSelected = async function(){
    ...
    if(current.type === 'microsoft') {
        ...
        // Офлайн-аккаунт не требует валидации.
        return true
    } else {
